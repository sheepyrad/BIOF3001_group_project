...
ug_data <- read.csv('../../data/mpox_UG.csv')

# Convert the 'Day' column to Date format
ug_data$Day <- as.Date(ug_data$Day, origin = '1899-12-30')

# Set the reference date (dateZero) as the earliest date in your data
dateZero <- min(ug_data$Day)

# Create a numeric representation of the days since dateZero
ug_data$Day_numeric <- as.numeric(ug_data$Day - dateZero) + 1

# Now, use the 'Daily cases' column that represents confirmed cases
counts_per_day <- data.frame(date = ug_data$Day_numeric, new_cases_onset = ug_data$Daily.cases)

# Handle missing dates by creating a complete sequence of days
days_sequence <- data.frame(date = seq(1, max(ug_data$Day_numeric)))
...
write.csv(counts_per_day, '../../output/UG/ug_mpox_epi_curve_onset_R.csv', row.names = FALSE)

# obtain the time series of the infection dates of all cases
caseReport_UG = counts_per_day$new_cases_onset;
caseReport_UG[caseReport_UG==0] = 1e-9; # to avoid calculation errors

source("DeconvolutionIncidence1.R")
deconvolInfection2 = DeconvolutionIncidence1(caseReport_UG,pdfConvolFG);
...
caseReport_UG_table = as.data.frame(cbind(1:length(caseReport_UG), caseReport_UG));
colnames(caseReport_UG_table) = c('date', 'number');
deconvol_table = as.data.frame(cbind(1:length(caseReport_UG), deconvolInfection2));
deconvol_table = head(deconvol_table, -5);
colnames(deconvol_table) = c('date', 'number');

ggplot()+ geom_line(data= caseReport_UG_table, aes(x=date, y=number),color = "red")+
  geom_line(data= deconvol_table, aes(x=date, y=number), color = 'blue')+theme_classic();

# save the result 
write.csv(deconvolInfection2, '../../output/UG/deconvolInfection_UG_R.csv',,row.names = FALSE)
