...
dateZero <- min(drc_data$Day)

# Create a numeric representation of the days since dateZero
drc_data$Day_numeric <- as.numeric(drc_data$Day - dateZero) + 1

# Now, use the 'Daily cases' column that represents confirmed cases
counts_per_day <- data.frame(date = drc_data$Day_numeric, new_cases_onset = drc_data$Daily.cases)

# Handle missing dates by creating a complete sequence of days
days_sequence <- data.frame(date = seq(1, max(drc_data$Day_numeric)))
...
write.csv(counts_per_day, '../../output/DRC/drc_mpox_epi_curve_onset_R.csv', row.names = FALSE)

# obtain the time series of the infection dates of all cases
caseReport_DRC = counts_per_day$new_cases_onset;
caseReport_DRC[caseReport_DRC==0] = 1e-9; # to avoid calculation errors

source("DeconvolutionIncidence1.R")
deconvolInfection2 = DeconvolutionIncidence1(caseReport_DRC,pdfConvolFG);
...
caseReport_DRC_table = as.data.frame(cbind(1:length(caseReport_DRC), caseReport_DRC));
colnames(caseReport_DRC_table) = c('date', 'number');
deconvol_table = as.data.frame(cbind(1:length(caseReport_DRC), deconvolInfection2));
deconvol_table = head(deconvol_table, -5);
colnames(deconvol_table) = c('date', 'number');

ggplot()+ geom_line(data= caseReport_DRC_table, aes(x=date, y=number),color = "red")+
  geom_line(data= deconvol_table, aes(x=date, y=number), color = 'blue')+theme_classic();

# save the result 
write.csv(deconvolInfection2, '../../output/DRC/deconvolInfection_DRC_R.csv',,row.names = FALSE)
